...
describe("shouldAnswer", () => {
  ...
  test("группа, рандом: попадание/промах по answer_prob", () => {
    const m = makeMsg({ chatType: "group", text: "болтовня" });
    stubRandom(0.05); // < 0.1
...
describe("tryQuickReply", () => {
  test("нет триггеров → false", async () => {
...
describe("handleChatMessage", () => {
  test("личка → отвечает (default-промпт), шлёт ответ модели", async () => {
    ...
    assert.ok(FETCH.of("/sendChatAction").length >= 1); // «печатает»
  });

  test("не отвечает, когда shouldAnswer=false", async () => {
  ...
  test("реплай → отвечает с reply-промптом (не вброс)", async () => {
    ...
    assert.ok(sys.includes("вчера был дождь")); // reply-промпт включает оригинал
  });
});
...
describe("tryCommand", () => {
  ...
  test("команда вернула null (например /admin не-админу) → молчит", async () => {
    ...
    assert.equal(ok, true);          // команда обработана
    assert.equal(FETCH.sends().length, 0); // но ничего не отправлено
  });
});


describe("аудит: chat flow — рандомные ветки", () => {
  test("группа: рандом + реплай → default-промпт (не вброс)", async () => {
